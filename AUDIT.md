...
- [ ] **Thread ancestors wiped + only scan page 1** — `_renderThreadPage`'s `innerHTML=` destroys ancestor rows from `_fetchThreadAncestors`, which also resolves parents only from the latest 50 channel txs. Resolve via `_fetchTxByHash`; store ancestors in state and re-render. *(M · thread)*
...
- [ ] **No in-flight guard on like/bookmark/follow/repost** — double-tap during the wallet round-trip fires the opposite action (double gas). Add a per-target pending set / disable the button. *(M · home/wallet)*
- [ ] **Offline retry republishes reactions / double-inserts rows** — only queue real posts/replies/quotes offline; de-dupe `_showPendingInFeed` against the published hash. *(M · wallet)*
